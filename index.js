...
async function getGeminiResponse(userId, userMessage) {
    try {
        ...
        const chat = model.startChat({
            ...
            systemInstruction: `Sen "${BOT_NAME}" adında eğlenceli, zeki bir WhatsApp grup botusun. Türkçe konuş, kısa ve eğlenceli cevaplar ver.`
        });
...
async function startBot() {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            console.log(`${delay/1000}sn sonra yeniden bağlanıyor... (deneme ${retryCount})`);
            setTimeout(startBot, delay);
        } else if (connection === 'open') {
            retryCount = 0;
            console.log(`\n✅ ${BOT_NAME} bağlandı ve hazır!\n`);
        }
    });
